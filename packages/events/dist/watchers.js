...
export async function getCircleEvents(client, circleAddress, fromBlock, toBlock) {
    ...
    const logs = parseEventLogs({ abi: circleAbi, logs: rawLogs });
    return logs
        .map((log) => ({
        name: (log.eventName ?? ''),
 ...
 */
export async function getCircleEventsChunked(client, circleAddress, fromBlock, toBlock, chunkSize = DEFAULT_CHUNK_SIZE) {
 ...
 */
export function watchCircleEvents(client, circleAddress, onEvent, opts = {}) {
    ...
    const tick = async () => {
        ...
        try {
            ...
            if (cursor === undefined)
                cursor = head; // default: only new events
            if (head >= cursor) {
